...
class RNNLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        # Apply linear layer for regression
...
class GTAMN_submodule(nn.Module):
    ...
    def forward(self, x):
        # Pass through each block in the block list
        for block in self.BlockList:
            x = block(x)

        # Apply final convolution layer
        x = self.final_conv(x.permute(0, 3, 1, 2)).squeeze(dim=-1)  # Squeeze to remove the time dimension
        ...
        # Reshape RNN output for the final regression MLP
        rnn_out = rnn_out.view(rnn_out.size(0), -1)

        # Final regression through MLP
