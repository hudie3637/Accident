...
def main():
    ...
    if torch.isnan(X_BERT).any() or torch.isnan(X_weekday).any() or torch.isnan(X_time).any() or torch.isnan(X_location).any() or torch.isnan(X_traffic).any():
        print("NaN found in loaded data")
    if X_BERT is None:
